...
class Parser : public tools, public node
{
            ...
            while(xx != type.size())
          	{           		
                ...
                if((type != "5") & (type != "6") & (type != "7"))
                {
                		...
                		tokens1[size] = new node(true,false,0,t,tp,typeName,pos,len,val);
	...
	Parser(){} 
		...
		int cond_expr1(int &i, int &tokenVal)
		{
			...
			{
				retVal=and_expr(i,tokenVal); // | found so go ahead;
			}
			if(retVal)
			{
				retVal=cond_expr1(i,tokenVal);
			}
		...
		int cond_expr(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"cond_expr");
			
			i++;

			retVal= and_expr(i,tokenVal);
		...
		int and_expr1(int &i,int &tokenVal)
		{
			...
			{
				retVal=cmp_expr(i,tokenVal); // & found so go ahead;
			}
			if(retVal)
			{
				retVal=and_expr1(i,tokenVal);
			}
		...
		int and_expr(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"and_expr");
			
			i++;

			retVal= eq_expr(i,tokenVal);
		...
		int eq_expr1(int &i,int &tokenVal)
		{
		...
		int eq_expr(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"eq_expr");
			
			i++;

			retVal= cmp_expr(i,tokenVal);
		...
		int cmp_expr1(int &i,int &tokenVal)
		{
			...
			{
				retVal=expr(i,tokenVal); // < found so go ahead;
			}
			if(retVal)
			{
				retVal=cmp_expr1(i,tokenVal);
			}
		...
		int cmp_expr(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"cmp_expr");
			
			i++;

			retVal= expr(i,tokenVal);
		...
		int opt_cond_expr(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"opt_cond_expr");
			
			i++;

			retVal=cond_expr(i,tokenVal);
		...
		int opt_assign(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"opt_assign");
			
			i++;

			retVal=assign_stmt(i,tokenVal);

			return retVal;
		}

		int opt_else(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"opt_else");
		...
		int while_stmt(node** root, int &i,int &tokenVal)
		{
			...
			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"while_stmt");
		...
		int for_stmt(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"for_stmt");
			...
			if(retVal)
			{
				retVal=opt_cond_expr(i,tokenVal);
			}
		...
		int if_stmt(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"if_stmt");
		...
		int assign_stmt(node** root, int &i, int &tokenVal)
		{
			...
			node *newNode= tokens1[tokenVal];
			//node *newNode= new node(true, false, 0, 0, 0, "",0,0,val1[tokenVal]);

			retVal=handleTokens(i, tokenVal,1);

			if(retVal)
			{
				(*root)->addChild(&newNode); //hi added.
				tokenVal++;
			}
			...
			if(retVal)
			{
				...
				node *newNode= new node(false, true, 0, 0, 0, "",0,0,"expr");
				(*root)->addChild(&newNode);

				retVal=expr(&newNode, i,tokenVal);
		...
		int output_stmt(int &i,int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"output_stmt");
		...
		int basic_expr(int &i,int &tokenVal)
		{

			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"basic_expr");
		...
		int paren_expr(int &i,int &tokenVal)
		{
			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"paren_expr");
			...
			if(retVal)
			{
				...
				node* newNode= new node(false, true, 0, 0, 0, "",0,0,"basic_expr");
				(*root)->addChild(&newNode);

				retVal=basic_expr(&newNode, i,tokenVal);
				if(retVal)
				{
					retVal=handleTokens(i, tokenVal, 34);
				}
			...
			{
				retVal=basic_expr(i,tokenVal);
				return retVal;
			}
			
		}

		int unary_expr(int &i, int &tokenVal)
		{
			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"unary_expr");
			...
			{
				node* newNode= new node(false, true, 0, 0, 0, "",0,0,"paren_expr");
				(*root)->addChild(&newNode);

				cout<<i<<i<<i<<i<<endl;
				retVal=paren_expr(&newNode, i,tokenVal);
		...
		int mult_expr1(node** root, int &i, int &tokenVal)
		{
			...
			else
			{
				...
				retVal=unary_expr(i,tokenVal); // * found so go ahead;
			}
			if(retVal==0)
			{
				return retVal;
		...
		{
			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"mult_expr");
			
			i++;

			//cout<<i<<i<<i<<i<<i<<endl;
			node* newNode= new node(false, true, 0, 0, 0, "",0,0,"unary_expr");
			(*root)->addChild(&newNode);

			retVal=unary_expr(&newNode, i,tokenVal);
			...
			{
				retVal= mult_expr1(&newNode, i,tokenVal);
			}
		...
		{	cout<<"expr1"<<endl;	
			...
			{
				retVal=mult_expr(i,tokenVal); // + found so go ahead;
			}
			if(retVal==0)
			{
				return retVal;
		...
		{
			//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"expr");
			
			i++;
			cout<<i<<endl;
			cout<<tokenVal<<endl;

			node* newNode= new node(false, true, 0, 0, 0, "",0,0,"mult_expr");
			(*root)->addChild(&newNode); //mult_expr added

			retVal=mult_expr(&newNode, i, tokenVal);
		...
		int stmt(node** root, int &i, int &tokenVal) 
		{
				...
				if(checkBound(tokenVal, i))
				{
					//parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"stmt");
				...
				switch(parseID)
				{
					case 9: //while
					{
						node* newNode= new node(false, true, 0, 0, 0, "",0,0,"while_stmt");
						(*root)->addChild(&newNode);

						retVal= while_stmt(&newNode, i,tokenVal);
					...
					case 8: //for
					{
						...
						retVal= for_stmt(&newNode, i,tokenVal);
					...
					{	
						...
						node* newNode= new node(false, true, 0, 0, 0, "",0,0,"assign_stmt");
						(*root)->addChild(&newNode);

						retVal= assign_stmt(&newNode, i,tokenVal);
			...
			int startParse(int &i, int &t)
			{
				node* newNode= new node(false, true, 0, 0, 0, "",0,0,"stmt");
				retVal= stmt(&newNode, i,t);
				return i;			
			}
			...
			void printRes(int ret)
			{
				...
				for(int z=0;z<ret;z++)
				{
					/*
					cout<<"<node>";
					if(!(parseNodes[z]->getTerm()))
					{
						cout<<"<nt>"<<parseNodes[z]->getVal()<<"</nt>"<<"<children>"<<endl;
						for(int e=0;e<=z;e++)
						{
							cout<<"	"; //for the gape
						}
					...
					{
						cout<<"<id>"<<tokens1[x]->getID()<<"</ID>"<<"<typenum>"<<tokens1[x]->getType()<<"</typenum>"<<"<typename>"<<tokens1[x]->getName()<<"</typename>"<<endl;
						cout<<"<position>"<<tokens1[x]->getPos()<<"</position>"<<"<length>"<<tokens1[x]->getLen()<<"</length>";
						if(tokens1[x]->getNoVal())
						{
							cout<<""<<endl;
						}
						else
						{
							cout<<"<value>"<<tokens1[x]->getVal()<<"</value>"<<endl;
						}
						cout<<"</node>"<<endl;
					}
...
int main(int argc, char *argv[]){
	...
	ret=par.startParse(parsPointer,tokenPointer);
	par.printRes(ret);
	par.deleteObj(ret);
