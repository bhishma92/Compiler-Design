...
class Parser : public tools, public node
{
	...
	Parser(){} 
		...
		int while_stmt(int &i,int &tokenVal)
		{
		...
		int assign_stmt(int &i, int &tokenVal)
		{
			parseNodes[i]= new node(false, true, 0, 0, 0, "",0,0,"assign_stmt");
		...
		int unary_expr(int &i, int &tokenVal)
		{
			...
			{
				cout<<i<<i<<i<<i<<endl;
				retVal=paren_expr(i,tokenVal);

				return retVal;
			}
		}

		int mult_expr1(int &i, int &tokenVal)
		{
			retVal=handleTokens(i, tokenVal, 18);
		...
		int mult_expr(int &i, int &tokenVal)
		{
